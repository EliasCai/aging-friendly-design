...
# Tab1 逻辑：图生文 → 图生图（模拟）
# ------------------------------------------------------------------
def fake_image_caption(pil_img: Image.Image) -> str:
    templates = [
...
def fake_image2image(pil_img: Image.Image, caption: str) -> Image.Image:
    W, H = pil_img.size
    img = pil_img.copy()
...
def ai_advise(image):
    ...
    advise = fake_image_caption(image)
    after = fake_image2image(image, advise)
    return advise, after
...
with gr.Blocks(css=css, title="颐居慧视") as demo:
    ...
    with gr.Tab("📸 环境评估"):
        ...
        gr.Examples(
            examples=[["examples/bedroom.jpg"], ["examples/kitchen.jpg"]],
            inputs=img_in,
            label="示例图片（点击即用）"
        )
    ...
    gr.Markdown("提示：本 Demo 使用模拟数据，仅用于交互演示。")
